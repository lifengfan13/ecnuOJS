...
import { getPageAllSubmissionByUserId } from '@/request/admin/api';
...
export default {
  ...
  methods: {
    handleEdit(id) {
      this.$router.push({ path: 'ProfileSubmissionDetail', query: { id: id }})
    },
    getAllSubmission(pageIdx) {
      ...
      getPageAllSubmissionByUserId(params).then(res => {
        if(res.status == 200) {
